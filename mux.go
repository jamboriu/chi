	...
	if rctx.URLPath != "" && r.URL.Path != rctx.URLPath {
		...
		if prefixLen >= 0 && prefixLen <= len(rctx.URLPath) {
			...
			if len(r.URL.Path) >= len(prefix) && r.URL.Path[:len(prefix)] == prefix {
				...
				numSlashes := strings.Count(prefix, "/")
				if numSlashes > 0 {
					targetSlash := numSlashes
					if len(rctx.RoutePath) > 0 && rctx.RoutePath[0] == '/' {
						targetSlash = numSlashes + 1
					}
					idx := ordinalIndex(r.URL.Path, "/", targetSlash)
					if idx != -1 {
						rctx.RoutePath = r.URL.Path[idx:]
					} else {
...
func ordinalIndex(s string, substr string, n int) int {
	if n <= 0 {
		return -1
	}
	idx := -1
	for i := 0; i < n; i++ {
		j := strings.Index(s[idx+1:], substr)
		if j == -1 {
			return -1
		}
		idx += j + 1
